...
namespace WindowsAppCommunity.CommandLine.Common.Entity.ExtendedDescription;

/// <summary>
/// Wacsdk set entity extended description command.
/// </summary>
public abstract class SetEntityExtendedDescriptionCommand : Command
{
    /// <summary>
    /// Initializes a new instance of the <see cref="SetEntityExtendedDescriptionCommand"/> class.
    /// </summary>
    public SetEntityExtendedDescriptionCommand(WacsdkCommandConfig config, string entityType, Option<string> repoOption, Option<string> idOption, Option<string> valueOption)
        : base("set", $"Sets the extended description for the {entityType}.")
    {
    ...
    public async Task InvokeAsync(string repo, string id, string value)
    {
        ...
        Logger.LogInformation($"Updating {nameof(entity.ExtendedDescription)}: {value}");
        await entity.UpdateExtendedDescriptionAsync(value, cancellationToken);
